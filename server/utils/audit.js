...
async function logAction(req, action, entity, entityId, details = null) {
    try {
        const userId = req.auditUserId || (req.user ? req.user.id : (req.body?.userId || req.query?.userId || null)); // Try to get from explicit override, auth middleware, body, or query params
